...
public class MainFragment extends Fragment implements OnMapReadyCallback, ReportDialog.DialogCallBack, GoogleMap.OnMarkerClickListener {
        ...
        switch (requestCode) {
            case REQUEST_CAPTURE_IMAGE: {
                if (resultCode == RESULT_OK && data != null && data.getExtras() != null) {
                    ...
                    File destination = new File(getActivity().getExternalFilesDir(null), "temp.png");
    ...
    private void uploadImage(final String key) {
        File file = new File(getActivity().getExternalFilesDir(null), "temp.png");
        if (!file.exists()) {
            Toast.makeText(getActivity(), "file not exist", Toast.LENGTH_LONG).show();
            dialog.dismiss();
            loadEventInVisibleMap();
            return;
        }
